...
extern std::unique_ptr<uint8_t> fasmMemoryBlock;

typedef FasmAssembleReturn(*Fasm_Assemble)(const char* src, uint8_t memoryBlock, int memoryBlockSize, int passes, uint32_t displayPipeHandle);
...
auto x = std::format(std::format_string(R"(
{}
{}
)"), 2, 3);